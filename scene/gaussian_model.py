...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        # Save without NaN positions
        xyz = self._xyz.detach().cpu().numpy()
        print("Number of Nan positions before saving ply: {}".format(np.count_nonzero(np.isnan(xyz[:, 0]))))
        valid_pos = 1-np.isnan(xyz[:,0])
        xyz = xyz[np.where(valid_pos)]

        normals = np.zeros_like(xyz)
        f_dc = self._features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_dc = f_dc[np.where(valid_pos)]
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = f_rest[np.where(valid_pos)]
        opacities = self._opacity.detach().cpu().numpy()
        opacities = opacities[np.where(valid_pos)]
        # Newly added for isotropic
        scale = self._scaling.detach().cpu().numpy()
        scale = scale[np.where(valid_pos)]
        rotation = self._rotation.detach().cpu().numpy()
        rotation = rotation[np.where(valid_pos)]
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                # Newly added for isotropic
                if not stored_state["exp_avg"].shape[1] == extension_tensor.shape[1]:
                    extension_tensor = extension_tensor.min(dim=1, keepdims=True)[0]
